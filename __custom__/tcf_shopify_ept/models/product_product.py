...
class ProductProduct(models.Model):
    ...
    def get_qty_on_hand(self, warehouse, product_list):
        on_hand_products = self.env['product.product'].with_context(dict(self._context, warehouse=warehouse.id)).read_group([('id', 'in', product_list.ids)], ['id'], ['id', 'free_qty'])
        onhand_list = []
    ...
    def get_forecated_qty(self, warehouse, product_list):
        va_products = self.env['product.product'].with_context(dict(self._context, warehouse=warehouse.id)).read_group([('id', 'in', product_list.ids)], ['id'], ['id', 'virtual_available'])
        va_list = []
        # This is needed to keep the rest of the modules working
